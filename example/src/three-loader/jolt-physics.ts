...
import { PhysicsShapeData, RigidBodyData, StaticBodyData } from './physics';
...
function createJoltShape(shape: Shape3DType, v3: Jolt.Vec3) {
  ...
  switch(shape.type) {
    case 'BoxShape3D':{
      ...
      return new Jolt.BoxShape(v3, 0.05);
    };
    case 'ConcavePolygonShape3D': {
      ...
      if('data' in p) {
        ...
        const j_shape = new Jolt.MeshShapeSettings(triangles, materials).Create().Get();
    ...
    case 'ConvexPolygonShape3D':{
      ...
      p.points.forEach(v => {
        hull.mPoints.push_back(unwrapVec3(<Vector3>v));
      })
      const j_shape = hull.Create().Get();
      return j_shape;
    };
    case 'SphereShape3D':{
    const p = shape.properties;
      DefaultSphereShape3D(p);
      return new Jolt.SphereShape(p.radius);
    }
    case 'CapsuleShape3D':{
      const p = shape.properties;
      DefaultCapsuleShape3D(p);
      return new Jolt.CapsuleShape(p.height/2, p.radius)
    }
    default:
      debugger;
  }
...
function createShape(shapes: PhysicsShapeData[], position: Jolt.Vec3, rotation: Jolt.Quat, scale: Jolt.Vec3, _tmp: Jolt.Vec3): Jolt.Shape | null {

  let j_shape: Jolt.Shape | null = null;
  if(shapes.length == 1) {
    ...
    if(j_shape && three_scale.x != 1.0) {
      ...
      j_shape = new Jolt.ScaledShape(j_shape, scale);
    }
  }
  if(shapes.length > 0) {
    ...
    shapes.forEach(shape => {
      ...
      if(j_shape)
        shapeSettings.AddShapeShape(position, rotation, j_shape, 0)
    });
    j_shape = shapeSettings.Create().Get();
  }
  return j_shape;
}

export function createRigidBody(jolt: JoltPhysics, scene: Scene, data: RigidBodyData): void {
  ...
  if(j_shape){
    ...
    unwrapQuat(three_quat, rotation);
    const creationSettings = new Jolt.BodyCreationSettings(j_shape, r_position, rotation, Jolt.EMotionType_Dynamic, LAYER_MOVING);
  ...
  const rotation = new Jolt.Quat();
    ...
    data.shapes.forEach(shape => {
      ...
      if(j_shape){
        ...
        unwrapRVec3(three_pos, r_position);
        unwrapQuat(three_quat, rotation);
        const creationSettings = new Jolt.BodyCreationSettings(j_shape, r_position, rotation, Jolt.EMotionType_Static, LAYER_NON_MOVING);
        const body = jolt.bodyInterface.CreateBody(creationSettings);
        Jolt.destroy(creationSettings);
