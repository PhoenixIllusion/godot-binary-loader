...
import { AnimationInstance, get_cache_type, Track, TrackType } from "./animation";
...
import { UpdateMode } from "@phoenixillusion/godot-scene-reader/process/scene/animation.js";
import { AnimationInstanceData } from "./animation/player_interface";

function is_variant_interpolatable(v: any): boolean {
  return false;
}
...
export class AnimationMixerInstance {
  ...
  get_animation(name: string) {
    return this.animations[name];
  }

  constructor(node: SceneInstance.Node) {
    const animationMixer = unwrap_properties_cached(node.properties) as AnimationMixer;
    DefaultAnimationMixer(animationMixer);
    Object.assign(this, animationMixer);

    const rootNode = navigate_nodepath<SceneInstance.Node>(node, this.root_node);
    if (rootNode == null) {
...
_blend_calc_total_weight() {
  ...
  _update_caches(): boolean {
    ...
    let reset_anim: AnimationInstance | null = null;
    ...
    for (const E of sname_list) {
      ...
      for (let i = 0; i < anim.tracks.length; i++) {
        ...
        let track: TrackCache | null = null;
        ...
        if (!track) {
          ...
          const track_xform = <TrackCacheTransform>track;
          ...
          const track_value = <TrackCacheValue>(track);
          ...
          if (track_src_type == 'value') {
            if (typeof track_value.init_value == 'string' && a_track.update != UpdateMode.UPDATE_DISCRETE) {
