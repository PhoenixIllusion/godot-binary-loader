...
import { AnimationNodeBlendSpace1D, AnimationNodeBlendSpace2D } from "../types/gen";
...
export class BlendSpace1D extends BlendSpace<number> {
  blend_mode!: AnimationNodeBlendSpace1D.BlendMode;
  value_label!: string;

  blend_position: number = 0;
  closest = -1;
  constructor(properties: AnimationNodeBlendSpace1D, name: string) {
    ...
    for (const [key, val] of Object.entries(blendSpace2DProperties)) {
      if (key.startsWith('blend_point_')) {
        const input = val as BlendPointRaw<number>;
  ...
  _process(p_playback_info: PlaybackInfo): NodeTimeInfo {
    ...
    let mind: NodeTimeInfo | undefined = undefined;
