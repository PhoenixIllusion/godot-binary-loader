...
import { BlendPoint, BlendPointRaw, BlendSpace, convertRaw } from "./blendspace";
...
function cross(a: ReadonlyVec2, b: ReadonlyVec2): number {
  return a[0] * b[1] - a[1] * b[0]
}

function is_point_in_triangle(s: ReadonlyVec2, a: ReadonlyVec2, b: ReadonlyVec2, c: ReadonlyVec2): boolean {
...
function get_closest_point_to_segment(p_point: ReadonlyVec2, p_segment: [ReadonlyVec2, ReadonlyVec2]): ReadonlyVec2 {
...
const distance_squared_to: (a: ReadonlyVec2, b: ReadonlyVec2) => number = vec2.squaredDistance;
const distance_to: (a: ReadonlyVec2, b: ReadonlyVec2) => number = vec2.distance;

function vec2_is_equal_approx(a: ReadonlyVec2, b: ReadonlyVec2): boolean {
  return is_zero_approx(a[0] - b[0]) && is_zero_approx(a[1] - b[1]);
}

function _blend_triangle(p_pos: ReadonlyVec2, p_points: [ReadonlyVec2, ReadonlyVec2, ReadonlyVec2], r_weights: [number, number, number]): void {
...
const ZERO: ReadonlyVec2 = [0, 0]

export class BlendSpace2D extends BlendSpace<ReadonlyVec2> {
  ...
  constructor(properties: AnimationNodeBlendSpace2D, name: string) {
    ...
    const blendSpace2DProperties = unwrap_property_paths(properties) as Record<string, any>;
    this.source_names = [];
    for (const [key, val] of Object.entries(blendSpace2DProperties)) {
      if (key.startsWith('blend_point_')) {
        const input = val as BlendPointRaw<Vector2>;
  ...
  _process(p_playback_info: PlaybackInfo): NodeTimeInfo {
    ...
    if (blend_mode == BLEND_MODE_INTERPOLATED) {
      ...
      let best_point: ReadonlyVec2 = ZERO;
      let first = true;
      ...
      for (let i = 0; i < triangles.length; i++) {
        ...
        for (let j = 0; j < 3; j++) {
          const s: [ReadonlyVec2, ReadonlyVec2] = [
            points[j],
            points[(j + 1) % 3]
          ];
