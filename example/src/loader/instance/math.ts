...
export function spherical_cubic_interpolate(res: quat, p_a: quat, p_b: quat, p_pre_a: quat, p_post_b: vec3, p_weight: number) {
...
export function spherical_cubic_interpolate_in_time(res: quat, p_a: quat, p_b: quat, p_pre_a: quat, p_post_b: vec3, p_weight: number, p_b_t: number, p_pre_a_t: number, p_post_b_t: number) {
	const from_q = quat.copy(tmp_q.from_q, p_a);
	const pre_q = quat.copy(tmp_q.pre_q, p_pre_a);
	const to_q = quat.copy(tmp_q.to_q, p_b);
	const post_q = quat.copy(tmp_q.post_q, p_post_b);
