...
interface ThemeItemElement extends XmlElement {
  name: 'theme_item';
  attributes: {
    name: string;
    type: string;
...
interface ThemeItemsElement extends XmlElement {
  name: 'theme_items';
  elements: ThemeItemElement[];
}
...
interface ClassElement extends XmlElement {
  ...
  elements: (ConstantsElement | MembersElement | ThemeItemsElement)[]
}
...
function readXML(file: ParsedTarFileItem) {
  ...
  const theme_items = xml.elements.find(x => x.name == 'theme_items')?.elements.filter(x => !members.find(m => m.name == x.name))
    .map(x => { const a = x.attributes; return { name: a.name, type: a.type, default: '' } }) || []
  ...
  return { name: name.replace('@', ''), inherits, members: [...members, ...theme_items], enums };
}
...
`
  ...
  classes.forEach(x => {
    ...
    addToType(x);
    defs += `${printJustType(x).replace(/\n\n\n/g,'\n\n')}\n${printEnums(x).replace(/\n\n\n/g,'\n\n')}\n`;
