...
export const UPDATE_CONTINUOUS = 0, UPDATE_DISCRETE = 1, UPDATE_CAPTURE = 2;
const vec3_interpolators = {
    _set: vec3.copy,
...
function try_position_track_interpolate(animation, track, p_time, result, p_backwards = false) {
    const t = animation.tracks[track];
    const ok = [false];
    _interpolate(result, animation.tracks[track].keys, p_time, t.interp, t.loop_wrap, ok, p_backwards, animation.loop_mode, vec3_interpolators);
    return ok[0];
}
function try_scale_track_interpolate(animation, track, p_time, result, p_backwards = false) {
    const t = animation.tracks[track];
    const ok = [false];
    _interpolate(result, animation.tracks[track].keys, p_time, t.interp, t.loop_wrap, ok, p_backwards, animation.loop_mode, vec3_interpolators);
    return ok[0];
}
function try_rotation_track_interpolate(animation, track, p_time, result, p_backwards = false) {
    const t = animation.tracks[track];
    const ok = [false];
    _interpolate(result, animation.tracks[track].keys, p_time, t.interp, t.loop_wrap, ok, p_backwards, animation.loop_mode, quat_interpolators);
    return ok[0];
}
const loc = [vec3.create(), vec3.create()];
const scale = [vec3.create(), vec3.create()];
const rot = [quat.create(), quat.create()];
function add_vec3_delta(dest, new_val, old_val, blend) {
...
function incremental_quat_delta(dest, new_val, old_val, blend) {
...
export function _blend_process(mixer, animation_instances, _p_delta, _p_update_only) {
