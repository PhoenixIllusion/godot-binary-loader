...
function _find(times, p_time, p_backward, p_limit = false) {
...
export function _interpolate(out, p_keys, p_time, p_interp, p_loop_wrap, p_ok, p_backward, loop_mode, funcs) {
        ...
        case INTERPOLATION_CUBIC_ANGLE:
            {
                ...
                if (p_interp == INTERPOLATION_CUBIC_ANGLE) {
                    return funcs._cubic_interpolate_angle_in_time(out, values[idx], values[next], values[pre], values[post], c, to_t, pre_t, post_t);
                }
                return funcs._cubic_interpolate_in_time(out, values[idx], values[next], values[pre], values[post], c, to_t, pre_t, post_t);
            }
            break;
        default:
            return funcs._set(out, values[idx]);
