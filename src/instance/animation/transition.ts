...
export class Transition extends SyncNode {
  ...
  constructor(properties: AnimationNodeTransition, name: string) {
    ...
    DefaultAnimationNodeTransition(transitionProperties as AnimationNodeTransition);
  ...
  _process(p_playback_info: PlaybackInfo): NodeTimeInfo {
    ...
    const get_input_name = (idx: number) => {
      return this.input_data[idx].name;
    }
    const find_input = (str: string) => this.input_index[str];
