...
function lerp(p_from: number, p_to: number, p_weight: number) { return p_from + (p_to - p_from) * p_weight; }
...
export function is_less_or_equal_approx(a: number, b: number) {
	return a < b || is_equal_approx(a, b);
}

export function is_less_approx(a: number, b: number) {
	return a < b && !is_equal_approx(a, b);
}

export function is_greater_or_equal_approx(a: number, b: number) {
	return a > b || is_equal_approx(a, b);
}

export function is_greater_approx(a: number, b: number) {
	return a > b && !is_equal_approx(a, b);
}


export function fposmod(n: number, d: number) {
	return ((n % d) + d) % d
}
...
export function signbit(value: number) {
	return value < 0 ? 1 : 0;
}
...
export function cubic_interpolate(p_from: number, p_to: number, p_pre: number, p_post: number, p_weight: number) {
	return 0.5 *
		((p_from * 2.0) +
			(-p_pre + p_to) * p_weight +
...
export function cubic_interpolate_in_time(p_from: number, p_to: number, p_pre: number, p_post: number, p_weight: number,
	p_to_t: number, p_pre_t: number, p_post_t: number) {
...
export function cubic_interpolate_vec3(res: vec3, p_a: vec3, p_b: vec3, p_pre_a: vec3, p_post_b: vec3, p_weight: number) {
...
export function cubic_interpolate_in_time_vec3(res: vec3, p_a: vec3, p_b: vec3, p_pre_a: vec3, p_post_b: vec3, p_weight: number, p_b_t: number, p_pre_a_t: number, p_post_b_t: number) {
...
function quat_exp(out: quat, q: ReadonlyQuat): quat {
	const [x, y, z] = q;
...
function quat_log(out: quat, q: ReadonlyQuat): quat {
	let [x, y, z, w] = q;
...
export function spherical_cubic_interpolate(res: quat, p_a: quat, p_b: quat, p_pre_a: quat, p_post_b: quat, p_weight: number) {
...
export function spherical_cubic_interpolate_in_time(res: quat, p_a: quat, p_b: quat, p_pre_a: quat, p_post_b: quat, p_weight: number, p_b_t: number, p_pre_a_t: number, p_post_b_t: number) {
	const from_q = quat.copy(tmp_q.from_q, p_a);
	const pre_q = quat.copy(tmp_q.pre_q, p_pre_a);
	const to_q = quat.copy(tmp_q.to_q, p_b);
	const post_q = quat.copy(tmp_q.post_q, p_post_b);
...
export function transform3d_to_mat4(transform: Transform3D, out: mat4 = mat4.create()): mat4 {
	mat4.identity(out);
...
const _lut: string[] = []; for (var i = 0; i < 256; i++) { _lut[i] = (i < 16 ? '0' : '') + (i).toString(16); }
